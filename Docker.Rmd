...
- *ports* tells Docker to map port 5000 inside the container to port 5000 on the host (published). Traffic sent to the Docker host on port 5000 will be redirected to port 5000 on the container. The app inside the container listents oon port 5000.
...
- *image: redis:alpine* tells Docker to start a standalone container called redis based n the redis:alpine image (will be pulled from Docker Hub).
- *netwrks*: the redis container will be attached to the counter-net network.
...
We can now see the images with *docker image ls*: 3 images were either built or pulled as part of the deployment. *docker container ls* shows 2 containers, *docker network ls* shows the network, and *docker voolume ls* the volumes.
...
We can bring down the app with the follwing command:
```{r}
# docker-compose down
```
...
The app can stopped without deleting its resources with
```{r}
# docker-compose stop
```
...
```
Finally, we can delete a stopped Compose app with with
```{r}
# docker-compose rm
```
...
![Image of the application](Images/DockerCompose.png)

This means we can make changes to files in the volume, from the host side, and have them reflected immediately in the app.
Example:
reprendre à use your favourite text editor...
