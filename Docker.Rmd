...
A container is the execution environment of an image. We can start one or more containers from an image. We can create our own images or start from an existing one (see <https://hub.docker.com/> example).
...
So far, it seems that the two models are quite identical. However, we see on the image above that each VM is a software that contains a virtual CPU, a virtual RAM, virtual storage disks, and so on. Those virtual resources are handled by an OS, and the problem is that every OS consumes CPU, RAM, storage, etc. Those OS also require a licence, patching, monitoring, and are vulnerable attack vectors...  
...
### Running containers
Let's see the commands to run containers. The following examples can be replicated on any Docker host or in Play with Docker (<https://labs.play-with-docker.com/>).
...
The command above starts a Linux container with Bash shell. The flag *-it* connects our terminal to the container's shell and makes it interactive. We didn't specify the *\<tag\>*, and it takes the *latest* image of ubuntu by default. We can check we're in Linux with the command *uname*, and in Ubuntu with *uname -v*. Finally, we asked to run Bash shell, that's why we can use commands such as *ls*, *cd*, and so on. However, some commands don't work because the image is optimized for containers and doesn't contain all the commands and packages... For example, the command *nano file.txt* won't work.

So, running containers is very easy, fast and platform-agnostic.


### Container processes
In the previous example, we started a container that runs Bash shell. It was the single and unique process running in the container. We can see it with the command *ps -elf*: there is ps -elf, but it is dead by the time it is displayed. Windows containers are a little bit different and run more processes.
...
```
If we execute *ps -elf* again, we now see 2 bash: this is because the previous command creates a new bash process to which we attach. If we exit this bash, it doesn't kill the container.
...
```
It is a good practice to proceed in two steps, although it is possible to do it with one command (*docker container rm -f*). This is to give a chance to the application to stop properly.
...
```
And check once again its presencee with *docker container ls*. It's not here... To see stopped container, we need to add the flag *-a*:
```{r}
# docker container ls -a
```
